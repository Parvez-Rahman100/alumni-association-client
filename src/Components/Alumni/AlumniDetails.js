...
const AlumniDetails = () => {
  ...
  useEffect(() => {
    const url = `https://alumni-association-server-56nn.vercel.app/alumnus/${alumniId}`;
    fetch(url)
      .then((res) => res.json())
    ...
    <div className="flex justify-center items-center py-60">
      <div className="card max-w-lg  formBG shadow-xl rounded-xl">
        <div className="card-body text-black">
          ...
          <h2 className=" ml-8 mt-4 font-bold">
            Registration Number : {details.alumni_registration_number}
          </h2>
          ...
          <p className=" ml-8 mt-4 font-semibold">
            Currently Working At : {details.connected}
          </p>
        </div>
        <div className="card-actions justify-end">
          <Link
            to="/alumnus"
            className="btn btn-outline w-10/12 mx-auto mb-4 text-black"
          >
            Back to Alumnus
          </Link>
        </div>
